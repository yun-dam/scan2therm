...
def identify_material(client: openai.OpenAI, label: str, image_b64: str, model: str = "gpt-4o-mini") -> str:
    ...
    resp = client.chat.completions.create(
        ...
        messages=[{
            ...
            "content": [
                ...
                {"type": "text", "text": (
                    f"This is a texture atlas from a 3D room scan. "
                    f"Identify the primary material of the '{label}' object visible in the texture. "
                    f"Reply with one word or short lowercase phrase only with no punctuation (e.g. 'wood', 'metal', 'fabric')."
                )},
            ],
        }],
        max_tokens=30,
...
def estimate_materials(
    ...
    for obj in seg_groups:
        ...
        if label.lower() in skip:
            continue
        results[oid] = {"label": label, "material": identify_material(client, label, image_b64, model)}
...
if not scan_dir:
    raise FileNotFoundError("No 3RScan_Data/*/semseg.v2.json + mesh.refined_0.png found")
