# From: https://github.com/ZiyuGuo99/Point-Bind_Point-LLM/blob/main/models/i2p_mae/modules.py
import torch
...
class Group_v2(nn.Module):  # FPS + KNN
    # Specially for I2P-MAE returning the orignal neighborhood coordinates
    def __init__(self, num_group, group_size):
        super().__init__()
        self.num_group = num_group
        self.group_size = group_size
    ...
    def forward(self, xyz):
        ...
        idx = idx.view(-1)
        neighborhood = xyz.view(batch_size * num_points, -1)[idx, :]
        neighborhood_ori = neighborhood.view(batch_size, self.num_group, self.group_size, 3).contiguous()
        # normalize
